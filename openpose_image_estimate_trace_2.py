...
def estimate_image(imagefile, model='cmu', path='', resize='432x368', plt_network=False,
    ...
    if cog != 'skip':
        ...
        #↑ここまでは、入力した静止画は一切変更されていない


   image = TfPoseEstimator.draw_humans(image, humans, imgcopy=False) #ここでボーンを加えている
    if not plt_network:
        fig = plt.figure(figsize=(int(w_pxl/200), int(h_pxl/200)))
        plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))#この時点で背景画像とボーンがくっついてる
        #↑上の行だけをコメントアウトすれば、背景画像やボーンが消える
        
        
        #ここからおそらく、各部位の重心位置のプロットのみ
        if cog != 'skip':
            ...
            plt.scatter(bodies_cog[:, 14, 0] * w_pxl, bodies_cog[:, 14, 1] * h_pxl, color="red", s=150) #重心14に散布図の点をうってる
            plt.vlines(bodies_cog[:, 6, 0] * w_pxl, ymin=0, ymax=h_pxl, linestyles='dashed',color='blue')
            plt.vlines(bodies_cog[:, 7, 0] * w_pxl, ymin=0, ymax=h_pxl, linestyles='dashed',color='red')
            plt.hlines(bodies_cog[:, 14, 0] * h_pxl, xmin=0, xmax=w_pxl, linestyles='dashed',color='green')  
            plt.vlines(bodies_cog[:, 0, 0] * w_pxl, ymin=0, ymax=h_pxl, linestyles='dashed',color='red')
            
            #bodies_cog[:, ポイント番号, 座標]   で数値を出力する
            #ポイント番号は0~14(0:頭部の重心14:全身の重心6:右足首の重心7:左足首の重心
            #座標は0にするとx座標、1にするとy座標
